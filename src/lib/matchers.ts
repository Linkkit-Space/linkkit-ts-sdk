...
import { LinkkitDefaultError } from "../models/errors/linkkit-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | LinkkitDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | LinkkitDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new LinkkitDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new LinkkitDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
